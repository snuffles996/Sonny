...
export async function classifyIntent(message: string): Promise<ClassificationResult> {
  ...
  const response = await client.messages.create({
    ...
    system: [
      ...
      `- Clear book searches: "find a book about X", "recommend a book on Y", "what book is about Z" → book_search`,
      `- Clear movie/TV lookups: "what is X", "look up X", "tell me about the show X", "find the movie X", "what movies are like X" → movie_query`,
      ``,
      `Set confidence: "low" and intent: "conversational" for:`,
      `- "What should I watch/read next" (needs library context to avoid duplicates)`,
      `- Finishing, watching, or reading something (may need library update)`,
      `- Questions involving their existing library or personal preferences`,
      `- Ambiguous requests, follow-up questions, or general conversation`,
      `- Anything unclear or requiring reasoning from context`,
      ``,
      `Intents:\n${INTENT_DESCRIPTIONS}`,
    ].join("\n"),
